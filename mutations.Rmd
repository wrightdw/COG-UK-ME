...
params: 
  ...
  dataset_date:     "2021-10-29" # if empty, defaults to today's date, otherwise enter date string
...
consortium_uk_delta_parent <-
  bind_rows(
    ...
    # TODO Delta non-AY.4 and non-AY.4.2 categories
    consortium_uk %>% filter(str_starts(lineage_full, "B.1.617.2") & !str_starts(lineage, "AY.4")) %>% 
      mutate(lineage = "Delta_minus_AY.4"),
    
    consortium_uk %>% filter(str_starts(lineage_full, "B.1.617.2") & !str_starts(lineage, "AY.4.2")) %>% 
      mutate(lineage = "Delta_minus_AY.4.2")
  ) 

# count sequences by day
lineages_days_uk_all <- 
  consortium_uk_delta_parent %>% dplyr::count(lineage, sample_date, name = "n_day")
...
mutations_uk_spike <- mutations_uk %>% filter(gene == 'S')
mutations_uk_spike <- mutations_uk %>%
  mutate(epi_week = as.numeric(as.character(epi_week)))
mutations_uk_spike <- mutations_uk_spike[,c('sequence_name', 'sample_date', 'epi_week',
                                'epi_date', 'lineage', 'adm1', 'gene',
                                'variant')]
viruses <- dplyr::summarise(group_by(mutations_uk_spike, sequence_name, sample_date, epi_week,
...
# edit 'mab_data' so that it can be read into plot
mab_data
names(mab_data)[1] <- 'sets'
...
for (i in 1:nrow(mab_data)) {
  if (mab_data$Cas[i] > mab_data$Imd[i]) {
    mab_data$pheno[i] <- 'Cas_1'
    
    if (mab_data$Cas[i] >= display_threshold) {
      mab_data$pheno[i] <- 'Cas_2'
    }
    
  } else if (mab_data$Cas[i] < mab_data$Imd[i]) {
    mab_data$pheno[i] <- 'Imd_1'
    
    if (mab_data$Imd[i] >= display_threshold) {
      mab_data$pheno[i] <- 'Imd_2'
    }
    
  }
}

# save mab metadata and mab
write_rds(mab_data, str_c("COG-UK/", params$dataset_date, "/mab_data.rds"))
write_rds(mab, str_c("COG-UK/", params$dataset_date, "/mab.rds"))
```
