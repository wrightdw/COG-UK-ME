...
params: 
  mutations_csv:  "cog_global_2021-02-09_mutations.csv"
  global_csv:     "cog_global_2021-02-09_public.csv"
  consortium_csv: "cog_global_2021-02-09_consortium.csv"
...
consortium_uk <- 
  ...
  select(-starts_with("adm2")) %>% # remove adm2 columns
  type_convert %>% 
  filter(country == "UK")  
...
mutations_uk <- 
  mutations %>%
  filter(country == "UK") %>% 
  mutate(across(c(epi_week, gene, position, adm1), as_factor)) %>%
  mutate(epi_week = fct_expand(epi_week, epi_levels) %>% fct_inseq) %T>% 
...
mutation_reference_counts <- 
  bind_rows(mutation_counts, reference_counts, mutation_counts_nations, reference_counts_nations) %T>%
...
database <- 
  ...
  type_convert %>% 
  mutate(anchor = str_c("<a href='", doi, "'target='_blank'>", citation,"</a>"), .keep = "unused") %>% 
  group_by(position, mutation) %>% 
  summarise(escape = escape %>% unique %>% sort %>% toString, 
            # citation = citation %>% unique %>% toString, 
            # doi = doi %>% unique %>% toString,
            anchor = anchor %>% unique %>% str_c(collapse = "<br>"),
            mab = mab %>% any,
            plasma = plasma %>% any,
...
database %<>% 
  full_join(lineage_distinct, by = c("mutation" = "variant")) %>% 
  full_join(lineages, by = c("mutation" = "variant")) %>% 
  full_join(uk_lineages, by = c("mutation" = "variant")) %>% 
  full_join(variants_uk,  by = c("mutation" = "variant")) %>% 
  full_join(variants_nations,  by = c("mutation" = "variant")) %>% 
  full_join(variants_uk_28,  by = c("mutation" = "variant")) %>% 
  full_join(variants_nations_28,  by = c("mutation" = "variant_28")) 

database %<>% 
  rename(`# Global Lineages associated with` = n_lineages, 
  ...
  mutate(across(c(mutation, support, domain), as_factor))
