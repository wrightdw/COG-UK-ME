...
dodgy_omicron <- 
  consortium_uk %>% 
  filter(lineage == "BA.1" & sample_date < "2021-11-01")
...
thera_data <- bind_rows(therapeutics_data(params$remdesivir_dir, 4393), 
                        therapeutics_data(params$nirmatrelvir_dir, 3267))
mutations_rem_uk <- 
  mutations_uk %>% 
  filter(gene == "orf1ab") %>% 
  mutate(across(position, as.character)) %>%
