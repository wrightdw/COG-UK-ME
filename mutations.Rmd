...
params: 
  ...
  ensovibep_dir:    "Ensovibep Mutants" 
  ...
  evol_annotation_json: "Evolutionary Selection/annotation.json"
  evol_summary_json: "Evolutionary Selection/summary.json"
  esm_dms_data_csv: "Evolutionary Selection/dms_data@13.csv"
---
...
consortium_uk <- 
  ...
  select(-gisaid_id, -submission_org_code, -root_sample_id, -(adm2:travel_history), NUTS1,
         ...
         -usher_lineages_version, -lineage # replace lineage with usher_lineage
         ) %>% # remove unused columns
  relocate(NUTS1,.after = adm1) %>% 
  rename(lineage = usher_lineage) %>% 
  type_convert 
...
# Sample of 100K sequences from full dataset for development purposes
...
dodgy_omicron <- 
  consortium_uk %>% 
  filter(str_starts(lineage_full, fixed("B.1.1.529.")) & sample_date < "2021-11-20")
...
```{r mutations_read, eval=FALSE, include=TRUE}
# read mutations from RDS to shortcut CSV parsing
mutations_uk <- str_c("COG-UK/", params$dataset_date, "/mutations_uk.rds") %>% read_rds
mutations_uk
```
...
deletions_nations <- 
  ...
  pivot_wider(names_from = adm1, values_from = n, names_expand = TRUE)
...
```{r}
# mutations_uk <- read_rds("COG-UK/2022-02-20/mutations_uk.rds")
# database_genome <- read_rds("COG-UK/2022-02-20/database_genome.rds")
...
mutations_s_escape_uk <- 
  mutations_uk %>% 
  filter(gene == "S") %>% 
  select(-gene, -cog_id, -NUTS1, -position) %>% 
  filter(variant %in% escape_mutations) %>%
  mutate(across(c(variant), fct_drop)) %T>% # drop non-spike and non-escape mutations from factor levels
  write_rds(str_c("COG-UK/", params$dataset_date, "/mutations_s_escape_uk.rds"))
```
...
therapeutics_data <- function(data_dir, position_polyprotein){
  fs::dir_ls(data_dir, glob = "*.csv") %>% # get file names
    map_dfr(read_csv, col_types = cols(quantification = col_character())) %>% 
    rename(gene = ORF) %>%
    rename_with(~str_c(.x, "_protein"), c(position, residue, mutation)) %>% 
    mutate(gene = recode(gene, "orf1b" = "ORF1ab", "ORF1a" = "ORF1ab"),
           position = position_protein + position_polyprotein - 1, .after = gene) %>% 
...
thera_data <- bind_rows(therapeutics_data(params$remdesivir_dir, 4393), # nsp12 
                        ...
                        therapeutics_data(params$ensovibep_dir, 1)) # S
...
```{r gff3}
library(janitor)
# See GFF3 specification https://github.com/The-Sequence-Ontology/Specifications/blob/master/gff3.md
...
# Calculate average entropy per epitope
avg_entropies <-
  database_tcell_predictions %>% 
  clean_names %>% 
  select(epitope, start_position, end_position) %>% 
  distinct %>% 
  fuzzy_inner_join(
      sergei_entropies_spike_all,
      by = c(
        "start_position" = "position",
        "end_position" = "position"),
      match_fun = list(
        `<=`, `>=`)) %>% 
  group_by(start_position, epitope) %>% 
  summarise(avg_entropy = mean(entropy), .groups = "drop")  

# Spike epitopes with mutations
epitopes_gff <- 
  database_tcell_predictions %>% 
  clean_names %>% 
  filter(gene == "S" & num_seqs_uk > 0) %>% 
  select(start_position, end_position, epitope) %>% 
  distinct %>% 
  inner_join(avg_entropies) %>% 
  mutate(seqid = "NC_045512.2", source = "RefSeq", type = "epitope", .before = 1) %>%
  mutate(score = ".", strand = "+", phase = ".", .after = end_position) %>%
  rename(attributes = epitope) %>%
  mutate(attributes = paste0("ID=", start_position, attributes, ";", "Name=", attributes, ";", "mean entropy=", avg_entropy)) %>%
  select(-avg_entropy) %>% 
  mutate(start = start_position * 3 + 21560, .after = start_position, .keep = "unused") %>%  # calculate genomic coordinates for spike start positions
  mutate(end = end_position * 3 + 21560 + 2, .after = end_position, .keep = "unused") # calculate genomic coordinates for spike end positions

# Spike epitope mutations referring to epitope parent ID
epitopes_mutations_gff <- 
  database_tcell_predictions %>% 
  clean_names %>% 
  filter(gene == "S" & num_seqs_uk > 0) %>% 
  inner_join(sergei_entropies_spike_all) %>% 
  select(start_position, epitope, position, mutation, num_seqs_uk, num_seqs_uk_28_days, entropy) %>% 
  distinct %>% 
  mutate(seqid = "NC_045512.2", source = "RefSeq", type = "amino_acid_substitution", .before = 1) %>% 
  mutate(start = position * 3 + 21560, .after = position) %>%  # calculate genomic coordinates for spike start positions
  mutate(end = position * 3 + 21560 + 2, .after = start, .keep = "unused") %>%  # calculate genomic coordinates for spike end positions
  mutate(score = ".", strand = "+", phase = ".", .after = end) %>% 
  rename(attributes = mutation) %>% 
  mutate(attributes = paste0("Parent=", start_position, epitope, ";", 
                             "Name=", attributes, ";", 
                             "sequences=", num_seqs_uk, ";", 
                             "sequences (28 days)=", num_seqs_uk_28_days, ";",
                             "entropy=", entropy), 
         .keep = "unused")

# Create GFF3 file
epitopes_gff %<>% 
  bind_rows(epitopes_mutations_gff) %T>% 
  write_tsv(str_c("COG-UK/", params$dataset_date, "/spike_epitopes.gff3"), col_names = FALSE) 
```
...
```{r vui_voc_other}
## Count VOC/VUI lineages by month, week and nation, including sublineages, for visualisation
# TODO count all lineages to include sublineages
# TODO fix colours in chart for missing lineages when changing time frame - include zero values?

# convert Delta and Omicron sublineages to parent for counting
consortium_uk_parent <-
  bind_rows(
    # convert non-Omicron VUI/VOC sublineages to parent 
    consortium_uk %>%
      filter(!str_starts(lineage_full, fixed("B.1.1.529.")) | 
               lineage == "BA.3" | str_starts(lineage_full, fixed("B.1.1.529.3."))) %>%
      mutate(lineage = gsub(sublineage_regex_gsub("AY"), "B.1.617.2", lineage)) %>% # convert AY.x to B.1.617.2
      mutate(lineage = gsub(sublineage_regex_gsub("B.1.177"), "B.1.177", lineage)) %>%  # convert B.1.177.x to B.1.177
      mutate(lineage = gsub(sublineage_regex_gsub("P.1"), "P.1", lineage)) %>%  # convert P.1.x to P.1
      mutate(lineage = gsub(sublineage_regex_gsub("B.1.351"), "B.1.351", lineage)), # convert B.1.351.x to B.1.351
  
    # convert non-BA.3 Omicron sublineages to parent including aliases using lineage_full
    consortium_uk %>% 
      filter(lineage == "BA.1" | str_starts(lineage_full, fixed("B.1.1.529.1."))) %>% 
      mutate(lineage = "BA.1"),
      
    # Special case for BA.2 minus BA.2.86/BA2.86.*
    consortium_uk %>% 
      filter(lineage == "BA.2" | str_starts(lineage_full, fixed("B.1.1.529.2."))) %>% 
      filter(lineage != "BA.2.86" & !str_starts(lineage_full, fixed("B.1.1.529.2.86."))) %>% # minus BA.2.86/BA2.86.*
      mutate(lineage = "BA.2"),
    
    # BA.2.86 / BA.2.86.*
    consortium_uk %>% 
      filter(lineage == "BA.2.86" | str_starts(lineage_full, fixed("B.1.1.529.2.86."))) %>% 
      mutate(lineage = "BA.2.86"),
    ...
    ## Special case for BA.5 and BQ.1. Report BA.5 as BA.5 minus BQ.1 ##
    # Report BA.5 as BA.5 minus BQ.1/BQ.1.*
    consortium_uk %>% 
      filter(lineage == "BA.5" | str_starts(lineage_full, fixed("B.1.1.529.5."))) %>% 
      filter(lineage != "BQ.1" & !str_starts(lineage_full, fixed("B.1.1.529.5.3.1.1.1.1.1."))) %>% # minus BQ.1/BQ.1.*
      mutate(lineage = "BA.5"),
    ...
    consortium_uk %>% 
      ...
      mutate(lineage = "XBB.1.5"),
    
    # CH.1.1 / CH.1.1.*
    consortium_uk %>% 
      filter(lineage == "CH.1.1" | str_starts(lineage_full, fixed("CH.1.1."))) %>% 
      mutate(lineage = "CH.1.1"),
    
    # XBB.1.16 / XBB.1.16.*
    consortium_uk %>% 
      filter(lineage == "XBB.1.16" | str_starts(lineage_full, fixed("XBB.1.16."))) %>% 
      mutate(lineage = "XBB.1.16"),
    
    # EG.5.1 / EG.5.1.*
    consortium_uk %>% 
      filter(lineage == "EG.5.1" | str_starts(lineage_full, fixed("XBB.1.9.2.5.1."))) %>% 
      mutate(lineage = "EG.5.1"),
    ####
...
### Precompute sublineage counts
```{r precompute}
# TODO create common helper functions include for both CLIMB and Shiny app
...
# Table of counts and reasons including sublineages
sublineages_reasons <- 
  bind_rows(
    ...
    sum_lineages(
      ...
      filter(variant == "sequences") %>%
      summarise(across(where(is.numeric), ~ sum(., is.na(.), 0))) %>% 
      mutate(lineage = "BA.2/BA.2.x (includes BA.2.75, BA.2.86 and other sublineages)",
    ...
    sum_lineages(
      ...
      filter(variant == "sequences") %>%
      summarise(across(where(is.numeric), ~ sum(., is.na(.), 0))) %>% 
      mutate(lineage = "BA.2.86/JN.x",
             reason =  "Alias of B.1.1.529.2.86. Denmark and Israel. WHO label: <strong>Omicron</strong>. Includes aliased JN.x sublineages of BA.2.86.")
  )%T>% write_rds(str_c("COG-UK/", params$dataset_date, "/sublineages_reasons.rds"))
```
...
geo_variants <- bind_rows(
  ...
  mutate(lineage = gsub(sublineage_regex_gsub("BA.2"), "BA.2", lineage)) %>% # convert BA.2.x lineages to BA.2  
  mutate(lineage = gsub(sublineage_regex_gsub("BA.4"), "BA.4", lineage)) %>% # convert BA.4.x lineages to BA.4  
  mutate(lineage = gsub(sublineage_regex_gsub("BA.5"), "BA.5", lineage)) %>% # convert BA.4.x lineages to BA.4  
...
scale_factor = 350 / 72 # scale up from 72 to 350ppi

png(filename=str_c("COG-UK/", params$dataset_date, "/Ronapreve.png"),
    width = 1800 * scale_factor, 
    height = 975 * scale_factor, 
    res = 72 * scale_factor,
    type = "cairo-png")
ronapreve_upset
dev.off()

png(filename = str_c("COG-UK/", params$dataset_date, "/Ronapreve_28.png"),
    width = 1800 * scale_factor, 
    height = 975 * scale_factor, 
    res = 72 * scale_factor,
    type = "cairo-png"
...
## Read JSON/ ESM-1b scores

```{r dms_integrated_data@13, eval=FALSE, include=TRUE}
suppressPackageStartupMessages(library(V8))

annotation_json = read_json(params$evol_annotation_json)
summary_json = read_json(params$evol_summary_json)
dms_antigenic <- read_csv(params$esm_dms_data_csv)
if("bloom_bind_avg" %in% colnames(dms_antigenic)) dms_antigenic <- dms_antigenic %>% select(-c("bloom_bind_avg")) # Drop column containing lots of NAs and causing trouble
if("bloom_expr_avg" %in% colnames(dms_antigenic)) dms_antigenic <- dms_antigenic %>% select(-c("bloom_expr_avg")) # Drop column containing lots of NAs and causing trouble
dms_antigenic <- toJSON(dms_antigenic)
```

## Javascript resources

```{r parse-json-data.js, eval=FALSE, include=TRUE}
ctx <- v8()
ctx$source("https://cdn.jsdelivr.net/npm/lodash@4.17.19/lodash.min.js")
ctx$source("https://d3js.org/d3.v4.min.js")
ctx$source("COG-UK/www/parse_evol_selection_data.js") # JS script with parsing functions
```

## Write integrated output

```{r dms_antigenic_res, eval=FALSE, include=TRUE}
# Call JS functions to parse both selection analysis and ESM results and return output of integrated CSV table
res = ctx$call("_main", annotation_json, summary_json, dms_antigenic)
dms_antigenic_res <- read_csv(res, col_names = TRUE)
# Assign selection labels
dms_antigenic_res %<>%
          mutate(evol_selection = case_when((conserved == "Yes" & positive == "Yes" & negative == "Yes") ~ "Conserved/ Positive/ Negative",
                                            (conserved == "Yes" & positive == "Yes") ~ "Conserved/ Positive",
                                            (positive == "Yes" & negative == "Yes") ~ "Positive/ Negative",
                                            (conserved == "Yes" & negative == "Yes") ~ "Conserved/ Negative",
                                            (conserved == "No" & positive == "No" & negative == "No") ~ "Undefined",
                                            conserved == "Yes" ~ "Conserved",
                                            positive == "Yes" ~ "Positive",
                                            negative == "Yes" ~ "Negative"
          )) %>% relocate(evol_selection, .before = entropy) %>% arrange(position)

dms_antigenic_res$entropy <- NULL # Better join the two datasets within the global.R file
dms_antigenic_res %T>% write_rds("dms_integrated_data@13.rds")
```

# RDS check
```{r rds_check, eval=FALSE, include=FALSE}
