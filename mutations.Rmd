...
lineage_distinct <- 
  ...
  summarise(n_lineages = n_distinct(lineage), n_uk_lineages =  n_distinct(uk_lineage), .groups = "drop") 
...
uk_lineages <-
  mutations_s_uk %>%
  distinct(position, variant, uk_lineage) %>%
  group_by(position, variant) %>%
  arrange(position, variant, uk_lineage, .by_group = TRUE) %>%
  summarise(uk_lineages = toString(uk_lineage), .groups = "drop")
...
database %<>% 
  ...
  full_join(uk_lineages, by = join_cols) %>% 
