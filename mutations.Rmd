...
params: 
  mutations_csv:  "cog_global_2021-01-22_mutations.csv"
  global_csv:     "cog_global_2021-01-22_public.csv"
  consortium_csv: "cog_global_2021-01-22_consortium.csv"
  spike_csv:      "spike_info_2021_01_19.csv"
---
```{r libs}
library(tidyverse)
library(magrittr)
...
positions_by_week <- 
  mutations_uk %>%
  group_by(epi_week, gene, position, .drop = FALSE) %>% # count all possible epi_week/gene/position combinations 
...
reference_counts <- 
  ...
  mutate(n = n_sequences - n_variant_sequences, variant = "REF", .keep = "unused") %>% # calculate reference / other
  filter(n > 0) # remove zero REF entries (where all sequences are variants for an epi_week / position combo)

mutation_counts <- 
  mutations_uk %>%
  count(epi_week, gene, position, variant)

mutation_reference_counts <- bind_rows(mutation_counts, reference_counts) %T>% write_rds("mutation_reference_counts.rds")
...
database <- 
  ...
  type_convert %>% 
  full_join(lineage_distinct, by = c("replacement" = "variant")) %>% 
  full_join(lineages, by = c("replacement" = "variant")) %>% 
  full_join(uk_lineages, by = c("replacement" = "variant")) %>% 
  full_join(variants_uk,  by = c("replacement" = "variant")) %>% 
  full_join(variants_nations,  by = c("replacement" = "variant")) %>% 
  full_join(variants_uk_28,  by = c("replacement" = "variant")) %>% 
  full_join(variants_nations_28,  by = c("replacement" = "variant_28")) 
...
database %<>% rename(`# Global Lineages associated with` = n_lineages, 
                    ...
                    `numSeqs NI 28 days` = `UK-NIR_28`
                            ...
                            across(starts_with( c("numSeqs", "#") ), ~replace_na(.x, 0L))
                          )
...
database %>% 
  ...
  select(replacement, `numSeqs UK`, `numSeqs UK 28 days`, `numSeqs Eng 28 days`, `numSeqs Scotland 28 days`, `numSeqs Wales 28 days`, `numSeqs NI 28 days`) %T>% write_csv("appendix.csv")
```
...
```{r escape}
escape <- 
  read_csv("list_escape_2.csv", col_names = "replacement")

database %>% 
  right_join(escape) %>% 
  select(replacement, `numSeqs UK`:`numSeqs Wales 28 days`) %T>% 
