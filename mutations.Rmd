...
params: 
  ...
  microreact_csv: "cog_global_2021-01-02_metadata_private.csv"
...
```{r microreact_read}
microreact_uk <- 
  read_csv(params$microreact_csv, col_types = cols(.default = col_character())) %>%  
  type_convert %>% 
  filter(country == "UK")
```
...
microreact_uk %>% filter(sample_date >= params$sample_date_28 & n501y == "Y" & del_21765_6 == "del")  %>% count(lineage) %>% arrange(desc(n))
```
```{r}
...
microreact_uk %>% filter(sample_date >= params$sample_date_28 & n439k == "K" & del_21765_6 == "del")  %>% count(lineage) %>% arrange(desc(n))
```
```{r}
...
consortium_uk <- consortium %>% filter(country == "UK")
```
```{r database}
...
# Sample dates in last 14 days
variants_nations_14 <- 
  mutations_s_uk %>% 
  filter(sample_date >= params$sample_date_14) %>% 
  count(variant, adm1) %>% 
  spread(adm1, n) %>% 
  rename_with(~paste0(., "_14"))

variants_uk_14 <- 
  mutations_s_uk %>% 
  filter(sample_date >= params$sample_date_14) %>% 
  count(variant, name = "UK_14")

database <- 
  read_csv("database for Dashboard - Sheet1.csv", skip = 1, col_types = cols(.default = col_character())) %>% 
  type_convert %>% 
  select(-X1) %>% 
  ...
  left_join(variants_uk_14,  by = c("replacement" = "variant")) %>% 
  left_join(variants_nations_14,  by = c("replacement" = "variant_14"))

database %<>% mutate(`# Global Lineages associated with` = n_lineages, 
                    ...
                    `numSeqs UK 14 days` = UK_14, 
                    `numSeqs Eng 14 days` = `UK-ENG_14`, 
                    `numSeqs Scotland 14 days` = `UK-SCT_14`, 
                    `numSeqs Wales 14 days` = `UK-WLS_14`) %>% select(-(n_lineages:`UK-WLS_14`))

database %T>% write_csv("database_2021-01-02.csv")
```
...
# mutations_s_uk %>% filter(variant == "N501Y") %>% ggplot(aes(x = sample_date)) %>% geom_bar()
...
```
```{r}
microreact_v <- microreact_uk %>% filter(n501y == "Y" & 
                                           del_21765_6 == "del" &
                                           sample_date >= "2020-11-24" &
                                           lineage == "B.1.1.7"
                                         )
...
```
```{r}
mutations_s_uk %>% summarise(n_distinct(variant))

```
```{r}
mutations_s_uk %>% count(variant) %>% filter(n == 1) 
```
```{r}
mutations_s_uk %>% count(variant) %>% filter(n >= 100) 
```

```{r table_1}
database %>% slice_max(`numSeqs UK`, n = 15) %>% select(replacement, `numSeqs UK`, `numSeqs UK 28 days`, `numSeqs Eng 28 days`, `numSeqs Scotland 28 days`, `numSeqs Wales 28 days`, `numSeqs NI 28 days`)
```
