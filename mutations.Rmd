...
lineages_days_uk_all <- 
  consortium_uk_delta_parent %>% dplyr::count(lineage, sample_date, adm1, name = "n_day")

# by week
lineages_weeks_uk_all <- 
 consortium_uk_delta_parent %>% dplyr::count(lineage, epi_date, adm1, name = "n_week") 
...
geo_variants <- 
  ...
  consortium_uk %>%
  filter(!is.na(NUTS1)) %>% 
  mutate(lineage = gsub("AY\\..*", "B.1.617.2", lineage)) %>%  # convert AY.x lineages to B.617.2
  mutate(across(lineage, as_factor)) %>%
...
geo_all <- 
  ...
  filter(epi_week >= 39) %>% # TODO first week VUIs and VOCs appeared - this is no longer correct!
  ...
  # write_csv("nuts1.csv") # data file for Observable / Vega Lite

geo_all <- 
  dplyr::inner_join(geo_all, epi_lookup) %T>% 
...
#   consortium_uk %>% 
#   filter(sequence_name %in% del_22289_6_samples) %>% 
#   dplyr::count(epi_week, NUTS1, name = "n", .drop = FALSE) %>% 
#   mutate(variant = "del243-244", .before = 1)

sequences_by_week_lineages <- 
  consortium_uk %>% 
  filter(!is.na(NUTS1)) %>%
  dplyr::count(epi_week,  lineage, NUTS1, name = "n", .drop = FALSE) # count sequences per epiweek and NUTS1
...
mutations_s_uk<- inner_join(mutations_s_uk, consortium_uk)
mutations_s_uk<- mutations_s_uk %>% filter(!is.na(NUTS1))
antigenic_mutations <- 
  mutations_s_uk %>% 
...
write_rds(str_c("COG-UK/antigenic_mutations_lineages_all.rds"))
antigenic_mutations_lineages_all
```
