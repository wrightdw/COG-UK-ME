...
params: 
  ...
  dataset_date:     "2021-11-09" # if empty, defaults to today's date, otherwise enter date string
...
geo_all <- 
  ...
  dplyr::mutate(Proportion = ifelse(n==0, 0, n_week / n)) %>%  # keep zero counts for rendering 
  select(-n) %>%
  inner_join(NUTS1_objectid) %>% 
  dplyr::rename(Count = n_week) %T>%
  write_csv("nuts1.csv")

geo_all<-inner_join(geo_all, epi_lookup)
...
##MAP
### Geo distribution
```{r geo_map_variant_alias, eval=FALSE, include=FALSE}
#Download some important packages
# Load Geo (above) first
...
shapefile<- readOGR(dsn = "COG-UK/Maps/", layer = "NUTS_Level_1_(January_2018)_Boundaries")

#Reshape for ggplot2 using the Broom package
mapdata <- shapefile  %>% tidy(region="nuts118nm") #This might take a few minutes

# as.factor(mapdataid) 
mapdata_1 <- mapdata %>% mutate(id = recode(id, 
