...
mutations_uk_spike <- mutations_uk_spike[,c('sequence_name', 'epi_week', 'variant')]
viruses <- dplyr::summarise(group_by(mutations_uk_spike, sequence_name, epi_week),
...
# edit a copy of 'mab_data' so that it can be read into plot
mab_metadata <- mab_data
...
mab_metadata$pheno <- NA
display_threshold <- 100
for (i in 1:nrow(mab_metadata)) {
  if (mab_metadata$Cas[i] > mab_metadata$Imd[i]) {
    mab_metadata$pheno[i] <- 'Cas_1'
    
    if (mab_metadata$Cas[i] >= display_threshold) {
      mab_metadata$pheno[i] <- 'Cas_2'
    }
    
  } else if (mab_metadata$Cas[i] < mab_metadata$Imd[i]) {
    mab_metadata$pheno[i] <- 'Imd_1'
    
    if (mab_metadata$Imd[i] >= display_threshold) {
      mab_metadata$pheno[i] <- 'Imd_2'
    }
...
write_rds(mab_metadata, str_c("COG-UK/", params$dataset_date, "/mab_metadata.rds"))
